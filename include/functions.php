<?php
  ...
  function rdioGet($args) {
    ...
    $rs = $db->query("SELECT `return` FROM api_usage WHERE api='rdio' AND params='".addslashes(json_encode($args))."' AND executed>=UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 24 HOUR)) ORDER BY executed DESC LIMIT 1");
